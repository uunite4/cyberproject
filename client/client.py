...
from map import screen, draw_map
from map_data import MAP
from protobufs.chat_pb2 import ChatMessagesList, ChatMessage
...
class Client:
    ...
    def __init__(self):
        ...
        # loginserver-server data client
    ...
    def on_receive(self, connection_id: int, data: bytes):
        ...
        if connection_id == self.chat_server_id:
            ...
            if (cmd == s.CMDS["ERROR"]):
                ...
                self.status_msg = ["OK"]
                self.running = False
            print(f"loginserver-server server: {self.status_msg}")
    ...
    def handle_chat_input(self, event):
        if event.key == pygame.K_LCTRL:
            ...
            if event.key == pygame.K_RETURN:
                ...
                self.chat_received_messages = self.chat_received_messages[-s.CHAT_MESSAGE_AMOUNT:]
                ...
                if str(event.unicode).isascii() and text_width <= s.CHAT_MAX_INPUT_WIDTH:
    ...
    def draw_chat(self):
        surface = pygame.Surface((s.CHAT_WIDTH, s.CHAT_HEIGHT))
        surface.fill(s.CHAT_BG_COLOR)

        y = 10
        for username, message in self.chat_received_messages:
            # Username
            name_surface = self.chat_font.render(f"{username}", True, s.CHAT_USERNAME_COLOR)
            surface.blit(name_surface, (10, y))

            # Message
            msg_surface = self.chat_font.render(message, True, s.CHAT_TEXT_COLOR)
        ...
        # Input Box
        input_box_height = 50
        pygame.draw.rect(surface, s.CHAT_INPUT_BG,
                         (0, s.CHAT_HEIGHT - input_box_height, s.CHAT_WIDTH, input_box_height))

        # Render input text inside input box with some padding
        input_surface = self.chat_font.render(self.chat_message, True, s.CHAT_TEXT_COLOR)
        input_pos = (10, s.CHAT_HEIGHT - input_box_height + 10)
        ...
        text_width, _ = self.chat_font.size(self.chat_message)
        if text_width >= s.CHAT_MAX_INPUT_WIDTH:
        ...
        pygame.draw.line(surface, (80, 80, 80), (0, s.CHAT_HEIGHT - input_box_height),
                         (s.CHAT_WIDTH, s.CHAT_HEIGHT - input_box_height), 2)
    ...
    async def run(self):
        ...
        print('connected to loginserver server')
        ...
        self.chat_server_id = await self.client.connect(
            server_ip=s.CHAT_SERVER_IP,
            server_port=s.CHAT_SERVER_PORT,
        )
