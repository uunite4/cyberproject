...
def create_massive_map():
    ...
    grid = [[' ' for _ in range(S.WIDTH)] for _ in range(S.HEIGHT)]

    # 1. יצירת המבנה המרכזי (b)
    start_x = (S.WIDTH - S.BUILDING_SIZE) // 2
    start_y = (S.HEIGHT - S.BUILDING_SIZE) // 2-80

    for y in range(start_y, start_y + S.BUILDING_SIZE):
        for x in range(start_x, start_x + S.BUILDING_SIZE):
            grid[y][x] = 'b'

    # 2. יצירת קבוצות אבנים (x)
    num_clusters = 200  # כמות קבוצות מפוזרות
    for _ in range(num_clusters):
        cx = random.randint(0, S.WIDTH - 1)
        cy = random.randint(0, S.HEIGHT - 1)

        # כל קבוצה מכילה כמה אבנים צמודות
        for _ in range(random.randint(10, 30)):
            nx = cx + random.randint(-1, 3)
            ny = cy + random.randint(-1, 2)
            if 0 <= nx < S.WIDTH and 0 <= ny < S.HEIGHT:
